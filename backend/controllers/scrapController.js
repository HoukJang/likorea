...
// 스크랩 토글 (추가/제거)
exports.toggleScrap = asyncHandler(async (req, res) => {
...
// 사용자의 스크랩 목록 조회
exports.getUserScraps = asyncHandler(async (req, res) => {
  const userId = req.user?._id;
...
// 특정 게시글의 스크랩 여부 확인
exports.checkScrapStatus = asyncHandler(async (req, res) => {
  const { postId } = req.params;
  const userId = req.user?._id;
...
// 게시글의 스크랩 수 조회
exports.getScrapCount = asyncHandler(async (req, res) => {
  const { postId } = req.params;
...
// 전체 스크랩 목록 조회 (관리자용)
exports.getAllScrapsAdmin = asyncHandler(async (req, res) => {
  // 관리자 권한 확인
